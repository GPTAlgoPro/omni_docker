...
PYTHON_BIN="${PYTHON_BIN:-$(which python3 2>/dev/null || echo "")}"
...
REGISTER_RESULT=$(curl -s -X POST "http://localhost:18021/api/inference/register" \
  -H "Content-Type: application/json" \
  -d "{\"ip\": \"$LOCAL_IP\", \"port\": $PORT, \"model_port\": $PORT, \"model_type\": \"$MODE\", \"session_type\": \"release\", \"service_name\": \"o45-cpp\"}" 2>/dev/null || echo "")
...
echo "   📡 后端 API:       http://localhost:18021"
