...
DEFAULT_CTX_SIZE = int(os.environ.get("CTX_SIZE", "4096"))
...
# 服务注册配置（默认为 localhost:18021 端口，设为空则不注册）
# 🔧 [本地联调] 注册到后端服务 18021 端口（修复Docker端口映射）
def _get_default_register_url():
    """获取默认注册地址（localhost:18021）"""
    # 推理服务在主机上运行，后端在Docker容器内
    # 主机访问Docker容器的映射端口时，应使用 localhost:18021
...
def register_service_node(port: int, duplex_mode: bool):
    ...
    注意：推理服务实际上同时支持 simplex 和 duplex 模式，
    所以我们需要注册两次，让后端能找到对应的服务。
    """
    if not REGISTER_URL:
    ...
    try:
        ...
        # 使用 host.docker.internal 让 Docker 容器能访问主机服务
        # 如果环境变量 USE_DOCKER_HOST 为 true，则使用 host.docker.internal
        use_docker_host = os.environ.get("USE_DOCKER_HOST", "true").lower() == "true"
        if use_docker_host:
            service_ip = "host.docker.internal"
        else:
            service_ip = get_local_ip()

        # 只注册simplex服务（因为推理服务可以同时处理simplex和duplex请求）
        # 后端会通过向这个port发送健康检查来维持服务状态
        model_type = "duplex" if duplex_mode else "simplex"  # 根据启动参数决定
        data = {
            "ip": service_ip,
            "port": port,
            "model_port": port,
        ...
        print(f"正在注册服务节点 ({model_type}): url={url}, data={data}", flush=True)
        response = requests.post(url, json=data, timeout=10)
        if response.status_code == 200:
            print(f"服务节点 ({model_type}) 注册成功: {response.text}", flush=True)
        else:
            print(f"服务节点 ({model_type}) 注册失败: HTTP {response.status_code}, 响应: {response.text}", flush=True)
    except Exception as e:
        import traceback
...
def send_heartbeat(port: int, service_ip: str):
    """定期发送心跳，保持服务在线状态"""
    if not REGISTER_URL:
        return

    try:
        url = f"{REGISTER_URL}/api/inference/heartbeat"
        # 同时为simplex和duplex发送心跳
        for idx, model_type in enumerate(["simplex", "duplex"]):
            service_id = f"{service_ip}:{port + idx}"
            data = {"service_id": service_id}
            response = requests.post(url, json=data, timeout=5)
            if response.status_code == 200:
                print(f"[心跳] {model_type} 服务心跳成功", flush=True)
    except Exception as e:
        pass  # 心跳失败不打印，避免日志过多


def heartbeat_loop(port: int, service_ip: str, interval: int = 10):
    """心跳循环线程"""
    print(f"[心跳] 启动心跳线程，间隔 {interval}秒", flush=True)
    while True:
        time.sleep(interval)
        send_heartbeat(port, service_ip)
...
async def lifespan(app: FastAPI):
    ...
    try:
        use_docker_host = os.environ.get("USE_DOCKER_HOST", "true").lower() == "true"
        service_ip = "host.docker.internal" if use_docker_host else get_local_ip()
        heartbeat_thread = threading.Thread(
            target=heartbeat_loop,
            args=(app.state.port, service_ip),
            daemon=True
        )
        heartbeat_thread.start()
    except Exception as e:
        print(f"心跳线程启动失败: {e}", flush=True)
